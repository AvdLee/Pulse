...
final class LoggerStoreExportTests: LoggerStoreBaseTests {
    ...
    func testPreconditions() throws {
        ...
        XCTAssertEqual(try context.fetch(RSNetworkTaskEntity.self).count, 6)
        XCTAssertEqual(try context.fetch(RSNetworkTaskProgressEntity.self).count, 0)
        XCTAssertEqual(try context.fetch(RSNetworkRequestEntity.self).count, 6)
        XCTAssertEqual(try context.fetch(RSNetworkResponseEntity.self).count, 6)
        XCTAssertEqual(try context.fetch(RSNetworkTransactionMetricsEntity.self).count, 6)

        let blobs = try context.fetch(RSLoggerBlobHandleEntity.self)
    ...
    func testExportPackageToArchiveWithPredicate() async throws {
        ...
        let blobKeys = try store.viewContext.fetch(RSLoggerBlobHandleEntity.self).map(\.key)
        ...
        XCTAssertEqual(try context.fetch(RSNetworkTaskEntity.self).count, 0)
        XCTAssertEqual(try context.fetch(RSNetworkTaskProgressEntity.self).count, 0)
        XCTAssertEqual(try context.fetch(RSNetworkRequestEntity.self).count, 0)
        XCTAssertEqual(try context.fetch(RSNetworkResponseEntity.self).count, 0)
        XCTAssertEqual(try context.fetch(RSNetworkTransactionMetricsEntity.self).count, 0)
    ...
    func testExportPackageToArchiveWithSessions() async throws {
        ...
        XCTAssertEqual(try context.fetch(RSNetworkTaskEntity.self).count, 3)
        XCTAssertEqual(try context.fetch(RSNetworkTaskProgressEntity.self).count, 0)
        XCTAssertEqual(try context.fetch(RSNetworkRequestEntity.self).count, 3)
        XCTAssertEqual(try context.fetch(RSNetworkResponseEntity.self).count, 3)
        XCTAssertEqual(try context.fetch(RSNetworkTransactionMetricsEntity.self).count, 3)
    ...
    func testExportArchiveToArchiveWithPredicate() async throws {
        ...
        let blobKeys = try archive.viewContext.fetch(RSLoggerBlobHandleEntity.self).map(\.key)
